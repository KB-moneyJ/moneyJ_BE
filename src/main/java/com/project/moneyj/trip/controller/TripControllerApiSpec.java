...
@Tag(name = "Trip Plans", description = "여행 플랜 API") // 1
@RequestMapping("/trip-plans")
public interface TripControllerApiSpec {

    @Operation(summary = "여행 플랜 생성", description = "새로운 여행 플랜을 등록합니다.") // 3. API 설명
    @ApiResponses(value = { // 4. 응답 케이스
    ...
    ResponseEntity<List<TripPlanListResponseDTO>> getUserTripPlans(
            // 6. @AuthenticationPrincipal 처리 (핵심)
    ...
    ResponseEntity<?> getPlanDetail(
            @Parameter(description = "조회할 플랜 ID") @PathVariable Long planId, // 7. 파라미터 설명
            @Parameter(hidden = true) @AuthenticationPrincipal CustomOAuth2User customUser);
